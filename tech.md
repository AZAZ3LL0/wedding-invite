...
| `demo.ping` | ручной вызов и старт воркера | `{ at, note }` | 1, без backoff |

`demo.ping` — временный топик стадии 0. Он существует, чтобы чек-лист §16 «pg-boss гоняет демо-джоб из воркера» проверялся прогоном, а не обещанием. `at` — ISO-строка со смещением, `note` — строка 1–200 символов. Хендлер ничего не пишет в БД и дедуплицирует по `${at}:${note}`, так что правило идемпотентности §6.3 распространяется и на него. Топик, его схема и хендлер удаляются в стадии 4, когда воркер начинает возить настоящие напоминания.
...
| `GET /api/health` | liveness для деплоя. 200 и `{ status: 'ok', db: 'up' }`, если `select 1` к базе прошёл; 503 и `{ status: 'degraded', db: 'down' }`, если нет. Тело читают healthcheck контейнера, шаг деплоя и внешний мониторинг, поэтому оно не меняется без бампа версии ядра |
...
**Переменные инфраструктуры.** `.env.example` описывает только то, что читает `lib/server/config.ts`. Compose и Caddy нуждаются в собственных переменных, и они намеренно не попадают в прикладной конфиг: приложение не должно падать при старте из-за отсутствия домена, а `config.ts` не должен знать о существовании Docker.

| переменная | кто читает | по умолчанию |
|---|---|---|
| `SITE_DOMAIN` | Caddyfile | нет, задаётся на VPS |
| `POSTGRES_USER` | сервис postgres | `wedding` |
| `POSTGRES_PASSWORD` | сервис postgres | `wedding`, в проде обязательно переопределяется |
| `POSTGRES_DB` | сервис postgres | `wedding` |

На VPS они лежат в том же файле `.env`: docker compose читает его и для подстановки переменных, и через `env_file`. Значения по умолчанию годятся только для разработки.
